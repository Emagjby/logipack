<script lang="ts">
	...
	let newStatus = $derived(form?.values?.new_status ?? "");
	...
	let officeInputValue = $derived.by(() => {
		const fromForm = form?.values?.office_id;
...
{#if data.result.state === "error"}
	...
	<div class="stagger stagger-1 flex flex-col items-center py-20 text-center">
		...
		{/if}
		<a
			href={`/${lang}/app/admin/shipments/${page.params.id}`}
...
{:else}
	...
	{@const isOfficeDisabled = newStatus !== "in_transit"}
	{@const isOfficeRequired = newStatus === "in_transit"}
	<div
		class="stagger stagger-3 mt-6 rounded-xl border border-surface-700/50 bg-surface-900 p-5"
	>
		<h2 class="mb-4 text-sm font-semibold text-surface-50">
		...
		{:else}
			...
			>
				...
				<div>
					<label
						for="new_status"
						class="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-surface-600"
					>
					...
					<select
						...
						class="w-full rounded-lg border border-surface-700/50 bg-surface-800 px-3 py-2 text-sm text-surface-200 transition-colors focus:border-accent/50 focus:outline-none focus:ring-1 focus:ring-accent/50"
					>
						<option value="" disabled
							>{$_("shipment.update.select_status")}</option
						>
						{#each availableStatuses as status (status)}
							<option value={status}
								>{$_(`shipment_status.${status}`)}</option
							>
						{/each}
					</select>
				</div>
				...
				<div class="sm:col-span-2">
					...
					<textarea
						...
						class="w-full rounded-lg border border-surface-700/50 bg-surface-800 px-3 py-2 text-sm text-surface-200 placeholder:text-surface-600 transition-colors focus:border-accent/50 focus:outline-none focus:ring-1 focus:ring-accent/50"
